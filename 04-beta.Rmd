...
stocks <- read.csv("Data/04_Beta/A2_dataset_01_Ex_Session.txt", header = T, sep = "\t")
SMI <- read.csv("Data/04_Beta/A2_dataset_03_Ex_Session.txt", header = T, sep = "\t")
rf <- read.csv("Data/04_Beta/A2_dataset_02_Ex_Session.txt", header = T, sep = "\t")

# Select only the columns of interest
stocks <- stocks %>% 
  dplyr::select(c(Date, Nestle_PS, Roche_Holding, Novartis_N, UBS_N)) %>% 
  dplyr::filter(Date > "1997-08-29")
  
SMI <- SMI %>% 
  dplyr::select(c(Date, SMI)) %>% 
  dplyr::filter(Date > "1997-08-29")
...
for (i in Stonks){
  ...
  predicted_alpha <- summary(lm(fit_roll_coefs[,2] ~ lag.xts(fit_roll_coefs[,2])))$coefficients[1,1]
  predicted_beta <- summary(lm(fit_roll_coefs[,2] ~ lag.xts(fit_roll_coefs[,2])))$coefficients[2,1]
  
  # Calculate RMSE
  predicted_rmse <- sqrt(mean(summary(lm(fit_roll_coefs[,2] ~ lag.xts(fit_roll_coefs[,2])))$residuals^2))
...
```{r}
# Read in the data
A4 <- read.csv("Data/04_Beta/A2_dataset_04_Ex_Session.txt", header = T, sep = "\t")
A5 <- read.csv("Data/04_Beta/A2_dataset_05_Ex_Session.txt", header = T, sep = "\t")
